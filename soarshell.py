...
if len(sys.argv) == 2:
    ...
    check = data.split(".")
    if check[1] != "spr":
        print("This compiler only works with .spr file")
        ...
        try:
            ...
            result, error = run(data)
            if error:
                print(error.as_string())
            else:
                print(result)
        except FileNotFoundError:
            print(f"SoarPro can't open {data!r}")

else:
    print("Welcome to SoarPro Version 1.0.0")
    ...
    while True:
        ...
        if text == "help":
            help = """
    Welcome to the SoarPro shell!
    If you are new to using SoarPro you can checkout the tutorial
    on github at https://github.com/SoarPro/SoarPro.
    ...
    need to use it for more complex stuff check out the compiler! 
        ...
        # This happens when the user inputs "about without quotes"
            ...
            about = """
    SoarPro is a compiled, high level and general purpose programming
    ...
    production standard!!
        ...
        # This happens when the user inputs "leave" it also stops the shell from running
            ...
            print("Thanks for using SoarPro... Have a nice day!")
            break
